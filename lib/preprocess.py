...
The output Pickle file is an object w/nfs_dongmin/prototype_densecap/prototype_densecap/data/densecap_splits.jsonh the following elements:
...
def captions_list(data):
    encoded_list = []
    lengths = []
    for img in data:
        for region in img['regions']:
            cap = region['phrase']
            cap = sentence_preprocess(cap)
            cap = cap.encode()
            encoded_list.append(cap)
...
def sentence_generation(region_gt, output_path):
    ...
    for i, regs in enumerate(tqdm(region_gt)):
        ...
        for j, reg in enumerate(reg_gt):
            sen = sentence_preprocess(reg['phrase'])
...
def main(args):
    ...
    sentence_generation(data, args.sentence_output)
    ...
    with h5py.File(args.h5_output, 'w') as f:
        ...
        captions_str = captions_list(data)
        f.create_dataset('captions_str', data=captions_str)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sentence_output',
                        ...
                        help='Path to output sentences pickle file')

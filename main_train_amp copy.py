...
import torch.distrbuted as dist
...
def main_worker(gpu, ngpu_per_node):
    #Distributed Data Parallel Initial
    global best_acc1
    args.gpu = gpu
    ...
    #Model Loading
    text_encoder = SentenceTransformer(args.text_encoder)
    text_encoder_pool = text_encoder.start_multi_process_pool(target_devices=[2,3])
    visual_encoder = densecap_resnet50_fpn(backbone_pretrained=True,
                                  feat_size=384,
                                  hidden_size=4096,
                                  max_len=30,
                                  ...
                                  box_detections_per_img=50,
                                  init_training_step='E')
    visual_encoder.cuda(args.gpu)
    visual_encoder = DistributedDataParallel(visual_encoder, device_ids=[args.gpu])
    visual_encoder.train()

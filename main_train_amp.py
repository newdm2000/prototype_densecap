...
def get_feature(args):
    ...
    val_loader = DataLoaderPFG(val_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
    ...
    box_features_list = []
    sentence_list = []
    for batch, (img, targets, info) in enumerate(tqdm(val_loader)):
        img = [img_tensor.to(device) for img_tensor in img]
        targets = [{k:v.to(device) for k, v in target.items()} for target in targets]
        result = model(img)
        
        caption_gt = [t["caps"] for t in targets]
        cap = []
        ...
        print(len(cap_embedding))
                    
        for i in range(len(result)):
            if result[i]["scores"] > threshold:
                box_features_list.append(result[i]["feats"])
        print(result[0])
...
def caption_train(args):
    ...
    train_loader = DataLoaderPFG(train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                                 pin_memory=True, collate_fn=DenseCapDataset.collate_fn)
    val_loader = DataLoaderPFG(val_set, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                                 pin_memory=True, collate_fn=DenseCapDataset.collate_fn)
    model = densecap_resnet50_fpn(backbone_pretrained=True,
...
def em_train(args):
    """
        training visual and text encoder with em algorithm
        E-step : text encoding and kmeans algorithm
        M-step : visual encoding and weight update
    """
    train_set = DenseCapDataset(args.img_root, args.data_path, args.lut_path, dataset_type='train')
    val_set = DenseCapDataset(args.img_root, args.data_path, args.lut_path, dataset_type='val')
    idx_to_token = train_set.look_up_tables['idx_to_token']
    ...
    text_encoder = SentenceTransformer(args.text_encoder, device=device)
    pool = text_encoder.start_multi_process_pool(target_devices=[1,2])
    text_encoder.eval()
    ...
    optimizer = torch.optim.Adam([{'params': (para for name, para in visual_encoder.named_parameters()
                                  ...
                                  {'params': (para for para in visual_encoder.roi_heads.box_describer.parameters()
                                              if para.requires_grad), 'lr': args.caption_lr}],
                                  lr=args.lr, weight_decay=args.weight_decay)
    #visual_encoder = nn.DataParallel(visual_encoder, device_ids=[0,1,2,3])
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        print(f"epoch: {epoch}")
        text_encoder.eval()
        cluster_result, _ = sentence_embedding(args, text_encoder, sentences, pool=pool, sen_emb_batch_size=2000, k_means_iter=40, n_cluster=1024)
        centroid = cluster_result['centroids']
        density = cluster_result['density']
        ...
        for batch, (img, targets, info) in enumerate(tqdm(train_loader)):
            ...
            writer.add_scalar('details/loss_objectness', losses['loss_objectness'].mean().item(), iter_counter)
            writer.add_scalar('details/loss_rpn_box_reg', losses['loss_rpn_box_reg'].mean().item(), iter_counter)
            writer.add_scalar('details/loss_classifier', losses['loss_classifier'].mean().item(), iter_counter)
            writer.add_scalar('details/loss_box_reg', losses['loss_box_reg'].mean().item(), iter_counter)
            iter_counter += 1
...
def save_model(visual_encoder, optimizer, iter_counter, flag=None):
    ...
    filename = os.path.join('model_params_caption', '{}_{}.pth.tar'.format(MODEL_NAME, flag))
...
def sentence_embedding(args, model, sentences, pool, sen_emb_batch_size, k_means_iter, n_cluster):
    print("sentence embedding...")
    embedding_sen = model.encode_multi_process(sentences, pool=pool, batch_size=sen_emb_batch_size)
...
def set_args():
    ...
    parser.add_argument('--em_step',
                        action='store_true',
                        help='text embedding model')
    parser.add_argument('--caption_step',
                        action='store_true',
                        help='text embedding model')
